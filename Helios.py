...
class HeliosDAC():
	def __init__(self,queuethread=True, debug=0):
		...
		# attempt to set SDK version, ignore failure if endpoint not present
		try:
			self.setSDKVersion()
		except usb.core.USBError:
			pass
	...
	def setSDKVersion(self, version = HELIOS_SDK_VERSION):
		# attempt to write SDK version, ignore errors if endpoint not available
		try:
	...
	def DoFrame(self):
		...
		try:
			...
			# ignore write errors if endpoint not available
			return
		t = time.time()
...
if __name__ == "__main__":
	...
	cal = a.generateText("_____________________", 20,20,scale=10)
	# print(cal)
	# a.plot(cal)
